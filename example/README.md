...
- Find/Replace `app_demo` to name of the flutter name of the app (ex: `my_spiffy_app`)
- Find/Replace `Burst Demo` to a label used to name the app on mobile Platforms (ex:`My Spiffy Mobile App`)
- Find/Replace `Burst Demo for Mac` to the name of your macOS app (ex: `My Spiffy macOS App`)
- Find/Replace `icodeforyou.com` to the project down for the product/customer (ex: `apple.com`)
- Find/Replace `com.icodeforyou` with the projects **com.icodeforyou** that the start of the apps' identifier (ex: `com.apple`)
- Find/Replace `ui_selection_burst_flutter` with bundle id (ex: `my_spiffy_app`) **[using the folder name is ideal]**
- Find/Replace `uiSelectionBurstFlutter` for Apple bundles as iOS/MacOS do like '_'. **[ideal is folder name to lower-camel case (ex: `mySpiffyApp`)]**
